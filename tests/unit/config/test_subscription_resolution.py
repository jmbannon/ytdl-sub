...
def _ensure_resolved_yaml(sub: Subscription, output_directory: str, preset_type: str, resolution_level: int) -> None:
    ...
    expected_out['output_options']['output_directory'] = str(Path(output_directory) / sub.name)
...
def test_resolution_original_tv_show(
    resolution_level: int, config_file: ConfigFile, tv_show_subscriptions_path: Path, output_directory: str,
):
