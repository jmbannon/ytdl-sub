...
The GUI image is based on :lsio-gh:`LSIO's docker-code-server image <\ >`. See its
documentation regarding environment variables and other details. Once running, open `the
web UI`_ to edit the `configuration`_ and run ``ytdl-sub``.
