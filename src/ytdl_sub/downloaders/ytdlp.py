...
@contextmanager
def ytdlp_downloader(ytdl_options_overrides: Dict) -> ytdl.YoutubeDL:
    """
...
def extract_info(ytdl_options_overrides: Dict, **kwargs) -> Dict:
    """
    ...
    **kwargs
    ...
    """
    with ytdlp_downloader(ytdl_options_overrides) as ytdlp:
...
def extract_info_with_retry(
    ...
    **kwargs
    ...
    while not entry_files_exist and num_tries < _extract_entry_num_retries:
        entry_dict = extract_info(ytdl_options_overrides=copied_ytdl_options_overrides, **kwargs)

        is_downloaded = is_downloaded_fn is None or is_downloaded_fn()
        is_thumbnail_downloaded = is_thumbnail_downloaded_fn is None or is_thumbnail_downloaded_fn()
        ...
        time.sleep(_extract_entry_retry_wait_sec)
        num_tries += 1
        ...
        if num_tries < _extract_entry_num_retries:
            download_logger.debug(
                "Failed to download entry. Retrying %d / %d",
                num_tries,
                _extract_entry_num_retries,
            )
...
def _get_entry_dicts_from_info_json_files(working_directory: str) -> List[Dict]:
    """
    Parameters
    ----------
    working_directory
...
def _listen_and_log_downloaded_info_json(working_directory: str, log_prefix: Optional[str]):
    """
...
def extract_info_via_info_json(
    ...
    **kwargs
    ...
    """
    try:
        with _listen_and_log_downloaded_info_json(
            working_directory=working_directory, log_prefix=log_prefix_on_info_json_dl
        ):
            _ = extract_info(ytdl_options_overrides=ytdl_options_overrides, **kwargs)
    ...
    return _get_entry_dicts_from_info_json_files(working_directory=working_directory)
